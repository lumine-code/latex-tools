...
function observedFilesSpec(mainModule) {
  return {
    ...
    confirm: ({ item }) => {
      // `confirm` is normalized `when: "always"`, so Enter reaches here with no
      // item whenever the list is empty or the query matches nothing. There is
      // nothing to open, so stay put rather than throwing at `item.filePath`.
