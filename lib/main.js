...
const { observedFilesSpec, activeObservedFilesSession } = require("./observed-list");
...
module.exports = {
  ...
  deactivate() {
    ...
    // The kernel owns the modal, but its spec closes over this module, so it
    // must not outlive it.
    const session = activeObservedFilesSession();
    if (session) {
      session.cancel("api");
    }
  },
  ...
  showObservedFiles() {
    atom.modals.open(observedFilesSpec(this));
  },
  ...
  clearCompileOnSaveFiles() {
    ...
    this.compileOnSaveFiles.clear();
    this.updateObservedFilesStatus();
    // Clearing can be triggered from the status bar while the list is up, so
    // the list has to re-read the (now empty) observed set.
    const session = activeObservedFilesSession();
    if (session) {
      session.refresh();
    }
